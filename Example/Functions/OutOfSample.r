out_of_sample_sequence<- function(Jacobiano, th, lm, al, ts_training, num_points,
                                  Regression.Kernel, ...){
  out_of_samp = c()
  coeff = list()
  ...
  for(j in 1:num_points){
    ...
    Jacobiano = update_Jacobian_TS(ts_training, TargetList, Embedding, th, lm, 
                                   RegressionType,al,Regression.Kernel)
...
update <- function(X, TargetList, Embedding, th, lm, RegressionType, al, Regression.Kernel){
  J = c0 = list()
  n_ = 1
  FUN = match.fun(RegressionType)
  for(df in TargetList){
    ########## Now compute the optimum regularized coefficients
    J[[n_]]  = FUN(X, df, Embedding, th[n_], lm[n_],al, Regression.Kernel)
    c0[[n_]] = J[[n_]]$c0
    J[[n_]] = J[[n_]][-1]
    n_ = n_ + 1
  }
...
######################################################################
update_Jacobian_TS <- function(X, TargetList, Embedding, th, lm, RegressionType,al,
                               Regression.Kernel){
  mine_output = update(X, TargetList, Embedding, th, lm, RegressionType,al,Regression.Kernel)
  ...
  for(k in 1:(nrow(X) - 1)){
    J[[k]] = do.call(rbind, lapply(1:ncol(X), function(x, M, i) unlist(M[[x]][i,]), 
                                   mine_output, k))
